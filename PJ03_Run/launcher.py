...
result_path = pathlib.Path(f'F:/amir/battle/cruse/{batch_size}_mse_{n_frames}_number_of_wav_data_{mini_batch}_lr{learning_rate}')

# specify the main path of your generated features, which are saved as .slv data
train_valid_test_path = pathlib.Path(f')

# specify the path of the noisy audio dataset for evaluation.
ref_dataset = pathlib.Path(f"")
